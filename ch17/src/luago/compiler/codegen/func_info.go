...
type funcInfo struct {
	...
	maxRegs   int
	scopeLv   int
	locVars   []*locVarInfo          //局部变量
	locNames  map[string]*locVarInfo //同名变量只保存当前作用域的变量
	breaks    [][]int
	parent    *funcInfo
	upvalues  map[string]upvalInfo
	insts     []uint32    //字节码
	subFuncs  []*funcInfo //子函数原型
	numParams int
	isVararg  bool
	//TODO
}
...
type locVarInfo struct {
	prev     *locVarInfo //同名的局部变量的单向链表
	name     string      //变量名
	scopeLv  int         //所在作用域的层次
	slot     int         //寄存器索引
	captured bool        //是否被闭包捕获
}

type upvalInfo struct {
	locVarInfo int //直接外网函数的局部变量索引
	upvalIndex int //直接外网函数的upvalues索引
	index      int //upvalue在当前闭包里索引
}
...
func (self *funcInfo) fixsBx(pc, sBx int) {
	i := self.insts[pc]
	i = i << 18 >> 18                  //消除sBx
	i = i | uint32(sBx+MAXARG_sBX)<<14 //重置sBx
	self.insts[pc] = i
}
